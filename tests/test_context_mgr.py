    ...
    def test_utc(self):
        ...
        with Timeline(100000):
            ...
            self.assertEquals(
                ...
                math.ceil(
                    (start_local - start_utc).seconds / 60.0 / 60.0
                )
            )
...
class TestTimelineContext(unittest.TestCase):
    ...
    def test_freeze_target(self):
        ...
        with Timeline() as timeline:
            ...
            self.assertRaises(
                AttributeError, timeline.freeze, date(1969, 1, 1)
            )
            ...
            self.assertAlmostEqual(
                int(
                    timedelta_to_seconds(
                        datetime.now() - datetime(2012, 12, 12, 1, 0, 0)
                    )
                ),
                (60 * 60 * 2) + 10
            )
    ...
    def test_fluent(self):
        ...
        with Timeline().scale(10).forward(120):
            self.assertTrue(
                int(timedelta_to_seconds(datetime.now() - start)) >= 120
            )
            time.sleep(10)
            self.assertTrue(
                int(timedelta_to_seconds(datetime.now() - start)) >= 130
            )
    ...
    def test_decorated(self):
        ...
        def _decorated():
            time.sleep(10)
            self.assertTrue(
                int(timedelta_to_seconds(datetime.now() - start)) >= 10
            )
        _decorated()
